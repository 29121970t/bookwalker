...
const booksById = computed(
  () => new Map(adminStore.books.value.map((book) => [book.id, book])),
);
...
const cartItems = computed(() =>
  Object.entries(cart.value)
    ...
    .filter(
      (item): item is { book: Book; quantity: number; lineTotal: number } =>
        Boolean(item),
    ),
);
...
const subtotal = computed(() =>
  Number(
    cartItems.value.reduce((sum, item) => sum + item.lineTotal, 0).toFixed(2),
  ),
);
const deliveryFee = computed(() =>
  subtotal.value >= 45 || subtotal.value === 0 ? 0 : 4.99,
);
const discount = computed(() => (subtotal.value >= 60 ? 8 : 0));
const total = computed(() =>
  Number((subtotal.value + deliveryFee.value - discount.value).toFixed(2)),
);
