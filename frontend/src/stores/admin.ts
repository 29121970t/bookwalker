...
function mapBook(book: ApiBook): Book {
  return {
    ...
    originalPrice:
      book.discountPrice == null ? undefined : Number(book.discountPrice),
    rating: 4.7,
    reviews: 0,
...
function mapAuthor(author: ApiAuthor): AdminAuthor {
  return {
    ...
    booksCount:
      author.booksCount ??
      books.value.filter((book) => (book.authorIds ?? []).includes(author.id))
        .length,
  };
}
  ...
  return {
    ...
    booksCount:
      ...
      books.value.filter((book) =>
        (book.publisherIds ?? []).includes(publisher.id),
      ).length,
  };
}
...
function mapTag(tag: ApiTag): AdminTag {
  return {
    ...
    usageCount:
      tag.usageCount ??
      books.value.filter((book) => (book.tagIds ?? []).includes(tag.id)).length,
  };
}
...
async function loadAll(force = false, includeRestricted = false) {
  ...
  loadPromise = (async () => {
    const [booksData, authorsData, publishersData, tagsData, genresData] =
      await Promise.all([
        ...
        apiRequest<Array<{ id: number; name: string; description?: string }>>(
          "/genres",
          { auth: false },
        ),
      ]);
...
async function updateOrder(id: number, payload: Record<string, unknown>) {
  ...
  orders.value = orders.value.map((order) =>
    order.entityId === id ? mapped : order,
  );
  return mapped;
}
...
async function updatePublisher(id: number, payload: Record<string, unknown>) {
  ...
  publishers.value = publishers.value.map((publisher) =>
    publisher.id === id ? mapped : publisher,
  );
  return mapped;
}
...
export function useAdminStore() {
  ...
  return {
    ...
    genres: computed(() => [
      "All",
      ...new Set(books.value.map((book) => book.genre)),
    ]),
    booksById: computed(
      () => new Map(books.value.map((book) => [book.id, book])),
    ),
    loadAll,
    createBook,
    updateBook,
