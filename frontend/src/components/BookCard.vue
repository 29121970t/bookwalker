...
<template>
  <Card
    class="py-0 group overflow-hidden rounded-3xl border-white/60 bg-white/90 shadow-lg shadow-slate-200/60 backdrop-blur-sm transition-transform duration-300 hover:-translate-y-1"
  >
    <CardContent class="space-y-4 p-4">
      <RouterLink :to="{ name: 'book', params: { id: book.id } }" class="block">
        <BookCover
          :book="book"
          compact
          class="transition-transform duration-300 group-hover:scale-[1.01]"
        />
      </RouterLink>
    ...
    <CardFooter
      class="flex items-center justify-between gap-3 border-t border-border/60 bg-slate-50/70 p-4"
    >
      <div class="space-y-0.5">
        <p class="text-lg font-semibold">${{ book.price.toFixed(2) }}</p>
        <p
          :class="[
            { 'opacity-0': !book.originalPrice },
